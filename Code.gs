 ...
 *    - ชีท "ActivityScores", "GroupScores", "Votes" ไม่ต้องสร้างเอง ระบบจะสร้างอัตโนมัติพร้อมหัวตาราง
 *      ตอนใช้งานฟีเจอร์ให้คะแนน / คะแนนกลุ่ม Iron Chef / โหวตเครื่องแต่งกาย เป็นครั้งแรก
...
const SHEET_GROUP_SCORES = 'GroupScores';       // คะแนนกลุ่ม Iron Chef: GroupName | Score | ApproverName | ScoreTime (1 แถวต่อ 1 กลุ่ม)
 ...
 *   คะแนนรวมของ "แต่ละคน" = คะแนนเดี่ยวรวมของคนนั้น + คะแนนกลุ่ม Iron Chef ของกลุ่มที่คนนั้นสังกัด
...
function handleGetGroupSummary(body) {
  ...
  _ensureSheet(SHEET_GROUP_SCORES, ['GroupName', 'Score', 'ApproverName', 'ScoreTime']);
  const groupScoreRows = _readAll(SHEET_GROUP_SCORES);
  const groupScoreMap = {};
  groupScoreRows.forEach(r => { groupScoreMap[String(r.GroupName).trim()] = r.Score; });
  ...
  validGroups.forEach(g => {
    groups[g] = {
      groupScore: (groupScoreMap[g] !== undefined && groupScoreMap[g] !== '') ? (Number(groupScoreMap[g]) || 0) : '',
      members: [],
      groupTotal: 0
    };
  });
  ...
  users.forEach(u => {
    ...
    const personalTotal = individualTotal + groupScoreNum;
  ...
  const ranking = validGroups
    .map(g => ({ group: g, groupScore: groups[g].groupScore, groupTotal: groups[g].groupTotal, memberCount: groups[g].members.length }))
...
/**
 * คืนคะแนนกลุ่ม Iron Chef ปัจจุบันของทุกกลุ่มจริง (อ่านชื่อกลุ่มจากชีต Users) — กลุ่มไหนยังไม่เคยให้คะแนนจะได้ score: ''
...
function handleGetGroupScores(body) {
  _ensureSheet(SHEET_GROUP_SCORES, ['GroupName', 'Score', 'ApproverName', 'ScoreTime']);
  const rows = _readAll(SHEET_GROUP_SCORES);
  const map = {};
  rows.forEach(r => { map[String(r.GroupName).trim()] = r; });
  ...
  validGroups.forEach(g => {
    const r = map[g];
    result[g] = r ? { score: r.Score, approverName: r.ApproverName || '', scoreTime: r.ScoreTime || '' } : { score: '', approverName: '', scoreTime: '' };
  });
  return { ok: true, groups: result, groupList: validGroups };
}

/**
 * ให้/แก้ไขคะแนนกลุ่ม Iron Chef ของกลุ่มใดก็ได้ตามชื่อจริงในชีต Users (แก้ซ้ำได้เรื่อยๆ เอาค่าล่าสุด)
...
function handleSaveGroupScore(body) {
  ...
  const sh = _ensureSheet(SHEET_GROUP_SCORES, ['GroupName', 'Score', 'ApproverName', 'ScoreTime']);
  const lock = LockService.getScriptLock();
  lock.waitLock(10000);
  try {
    const data = sh.getDataRange().getValues();
    ...
    sh.getRange(targetRow, 1, 1, 4).setValues([[group, score, body.approverName || '', new Date()]]);
  } finally {
    lock.releaseLock();
  }
 ...
 * - ironChefScore: คะแนนกลุ่ม Iron Chef ของกลุ่มที่คนนี้สังกัดอยู่ (ตามคอลัมน์ Group ในชีต Users)
 * - total: individualTotal + ironChefScore
 */
function handleGetScoreSummary(body) {
  ...
  if (myGroup) {
    _ensureSheet(SHEET_GROUP_SCORES, ['GroupName', 'Score', 'ApproverName', 'ScoreTime']);
    const groupRows = _readAll(SHEET_GROUP_SCORES);
    const groupRow = groupRows.find(r => String(r.GroupName).trim() === myGroup);
    if (groupRow && groupRow.Score !== '' && groupRow.Score !== undefined) ironChefScore = Number(groupRow.Score) || 0;
  }
